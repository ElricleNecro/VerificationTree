...
double** CalcLogDensite(const TNoeud root, const int NbBin, const double rmin, const double rmax, const double rnorm)
{
	...
	lr[0]           = log10((rmin - 1.0e3)/rnorm);
	lr[NbBin]       = log10(rmax/rnorm);
	cte             = ( lr[NbBin] - lr[0] ) / NbBin;

	printf("(%g -> %g) ; (%g -> %g)\n", (rmin - 1e3)/rnorm, lr[0], rmax/rnorm, lr[NbBin]);
	...
	for(int i = 0; i < NbBin; i++)
		densite[i] *= 3.0 * root->first[0].m / ( 4.0 * PI * ( pow(10.0, 3.0*lr[i+1])*rnorm /*lr[i+1]*lr[i+1]*lr[i+1]*/ - pow(10.0, 3.0*lr[i])*rnorm /*lr[i]*lr[i]*lr[i]*/ ) );
